...
} from '../utils/shopFilterHelpers.js';
...
function hasActiveFilters(params, filterMeta) {
  const { era, category, q, size, color, price_min, price_max } = params;
  const range = filterMeta?.priceRange || { min: 0, max: 0 };
...
function applyClientFilters(products, params) {
  const min = params.price_min != null && params.price_min !== ''
    ? +params.price_min
    : null;
  const max = params.price_max != null && params.price_max !== ''
    ? +params.price_max
    : null;
  const colors = parseColors(params.color);

  return products.filter((p) => {
    const price = getProductListPrice(p);
    if (min != null && price < min) return false;
    if (max != null && price > max) return false;
    if (!productMatchesSize(p, params.size)) return false;
    if (!productMatchesColors(p, colors)) return false;
    return true;
  });
}
...
function renderSidebar(sidebarWrap, params, filterMeta, closeSidebar) {
  if (!sidebarWrap) return;

  const { priceRange } = filterMeta;
  const activeColors = parseColors(params.color);
  const priceMin = params.price_min ? +params.price_min : priceRange.min;
  const priceMax = params.price_max ? +params.price_max : priceRange.max;
  ...
  ShopSidebar.bind(sidebarWrap, {
    ...
    onApply: (filters) => {
      Router.go('/shop', buildFilterParams(params, {
        ...
        price_min: filters.priceMin > priceRange.min ? filters.priceMin : '',
        price_max: filters.priceMax < priceRange.max ? filters.priceMax : '',
      }));
  ...
  const clearBtn = sidebarWrap.querySelector('#clear-filters');
  if (clearBtn) clearBtn.classList.toggle('hidden', !hasActiveFilters(params, filterMeta));
}
...
Router.onEnter('shop', async function (params) {
  ...
  if (sidebarWrap) {
    sidebarWrap.innerHTML = `<aside class="shop-sidebar w-full md:w-56 lg:w-64 shrink-0 text-sm text-muted">${t.loading}</aside>`;
  }
  ...
  try {
    ...
    const filterMeta = deriveShopFilters(allProducts);
    renderSidebar(sidebarWrap, params, filterMeta, closeSidebar);
