...
import { getHeroImagesFromSettings } from '../utils/heroImages.js';
...
export function mergeStoreSettings(remote) {
  ...
  const heroImages = getHeroImagesFromSettings(remote, {
    image: storeConfig.hero.image,
    name: storeConfig.name,
  });

  if (heroImages.length) {
    storeConfig.hero.images = heroImages;
    storeConfig.hero.image = heroImages[0].url;
  }
  ...
  if (remote.enamad_html != null) {
    storeConfig.enamad.html = remote.enamad_html || '';
  } else if (remote.enamad_url != null) {
    storeConfig.enamad.html = remote.enamad_url || '';
  }
