...
function _isVariableSetupIncomplete(productType, product) {
  if (productType !== 'variable') return false;
...
function _texts() {
  return {
    ...
    variantSetupIncomplete: _t('products.variantSetupIncomplete', 'این محصول چند واریانتی است اما سایز/رنگ تولید نشده. محورها را انتخاب کنید و «تولید واریانت‌ها» را بزنید، سپس موجودی هر واریانت را تنظیم کنید.'),
    variantSetupRequired: _t('products.variantSetupRequired', 'برای انتشار محصول چند واریانتی، ابتدا واریانت‌های سایز/رنگ را تولید کنید.'),
  };
}
  ...
  panel.innerHTML = `
    ...
    <div class="product-tab-panel ${_activeTab === 'variants' ? '' : 'hidden'}" data-tab-panel="variants">
      ${VariantsTab.render({
        ...
        productType: _currentProduct?.product_type || 'simple',
        variantSetupIncomplete: _currentProduct?.variant_setup_incomplete || false,
        t,
      })}
    </div>
    ...
    VariantsTab.bind(variantsPanel, {
      onGenerate: async (axes) => {
        ...
        try {
          ...
          _currentProduct.variant_setup_incomplete = false;
          VariantsTab.updateTable(variantsPanel, variants, t, 'variable');
...
export function initProductEditor() {
  ...
  document.getElementById('productForm')?.addEventListener('submit', async (e) => {
    ...
    const isActiveVariable = general.product_type === 'variable'
      && (general.status || _currentProduct?.status || 'active') === 'active';
    if (isActiveVariable && _isVariableSetupIncomplete(general.product_type, _currentProduct)) {
      window.toast?.(_t('products.variantSetupRequired', 'برای انتشار محصول چند واریانتی، ابتدا واریانت‌های سایز/رنگ را تولید کنید.'), 'error');
      _switchTab('variants');
      return;
    }
